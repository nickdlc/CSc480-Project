...
public:
    ...
    void receive_coeffs(ZZ_p coeff) { this->coeffs.push_back(coeff); }
    vector<ZZ_p> get_coeffs() { return this->coeffs; }
    void set_my_share();
    ZZ_p get_my_share() { return this->my_share; }

private:
    // if true, send random value to the others instead of using my_poly(i)
