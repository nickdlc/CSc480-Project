...
class Member {
    public: 
        ...
        Member(const int index, unsigned int deg);
    ...
    private:
        ...
        float my_share; //one's own share of their little function evaluated at their index which is index = self.index - 1    
        float big_function_evaluated;//big function evaluated at our index
        map<Member, mpz_t*> big_function_evaluated_by_others; // stores the evaluation of the big function by the members in the group
        float big_secret; //our computation of the secret being communicated to the group 
        map<unsigned int, mpz_t*> shares_for_others; //stores to then distribute all shares you compute for the other members in the group based on your little function.
        map<unsigned int, mpz_t*> shares_from_others; //stores all shares the other members of the group will send you
        map<unsigned int, coefficients_t > other_coefficients; //store the coefficients vector of the others members for phase 2
        Function my_function; //stores the coefficients of the member's polynomial (little function)
